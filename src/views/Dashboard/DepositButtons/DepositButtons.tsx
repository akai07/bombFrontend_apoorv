...
export const DepositButtons: FC<Props> = memo(function PurchaseButtons(props = {}) {
  ...
  const [approveStatus, approve] = useApprove(bombFinance.BSHARE, Treasury.address);
  const shoot = () => {
    alert("Great Shot!");
  }
  ...
  <button className={`${classes.root} ${props.className || ''}`} onClick={() => catchError(approve(), `Unable to approve `)} >
      ...
      <div className={`${classes.purchase} ${props.classes?.purchase || ''}`}>Deposit</div>
